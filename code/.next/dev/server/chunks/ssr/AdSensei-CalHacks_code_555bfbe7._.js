...
async function generateAnalysis(file, imageData) {
    try {
        ...
        const sentiment = mapSentiment(aiResult.sentiment);
        const colors = parseColors(aiResult.detected_elements?.colors || aiResult.audio_visual_signals?.color_palette || []);
        return {
            id: data.id || Math.random().toString(36).substr(2, 9),
            fileName: file.name,
            fileSize: file.size,
            uploadedAt: Date.now(),
            sentimentScore: calculateSentimentScore(aiResult.sentiment, aiResult.emotional_impact),
            sentiment,
            predictedCTR: aiResult.viral_potential / 20 || 3.5,
            engagementScore: aiResult.shareability_score || aiResult.emotional_impact || 75,
            dominantColors: colors,
            recommendations: aiResult.what_to_improve || aiResult.virality_tips || [],
            imageData
        };
    } catch (error) {
...
function DashboardPage() {
    ...
    const stats = analyses.length > 0 ? {
        avgSentiment: (analyses.reduce((sum, a)=>sum + (a.sentimentScore || 0), 0) / analyses.length).toFixed(1),
    ...
    const sentimentData = analyses.length > 0 ? [
        {
            name: 'Positive',
            value: Math.round(analyses.filter((a)=>a.sentiment === 'Positive').length / analyses.length * 100),
            fill: 'hsl(var(--chart-1))'
        },
        {
            name: 'Neutral',
            value: Math.round(analyses.filter((a)=>a.sentiment === 'Neutral').length / analyses.length * 100),
            fill: 'hsl(var(--chart-2))'
        },
        {
            name: 'Negative',
            value: Math.round(analyses.filter((a)=>a.sentiment === 'Negative').length / analyses.length * 100),
    ...
    const topAds = [
        ...analyses
    ].sort((a, b)=>(b.sentimentScore || 0) - (a.sentimentScore || 0)).slice(0, 3);
...
//# sourceMappingURL=AdSensei-CalHacks_code_555bfbe7._.js.map